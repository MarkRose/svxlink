...
void LinkManager::addLogic(LogicBase *logic)
{
  ...
  for (SinkMap::iterator it=sinks.begin(); it != sinks.end(); ++it)
  {
    ...
    // Skip the newly-added logic itself: Loop 1 above already created the
    // self-connection; a second pass would orphan the first valve/amp in the
    // splitter and add a spurious source to the mixer.
